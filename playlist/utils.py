...
def speed_to_bpm(speed):
    ...
    This is pulled from www.stepcal.com/mph-to-steps-calculator and assumes an average height of 170cm (5'7").
    """
    if speed <= 0:
        raise ValueError("Speed must be greater than zero.")
    elif speed <= 4:
        # Walking speed (0-4 mph)
        bpm = speed * 1609.34 / 60 / (170 * 0.414 / 100)
    elif speed <= 12:
        # Running speed (4-12 mph)
        bpm = speed * 1609.34 / 60 / (170 * 0.45 / 100)
    else:
        raise ValueError("Speed must be less than or equal to 12 mph.")
...
def bpm_to_speed(bpm):
    ...
    if bpm <= 0:
        ...
        raise ValueError("BPM must be less than or equal to 200.")
